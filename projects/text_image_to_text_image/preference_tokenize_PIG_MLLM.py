...
from align_anything.models.chameleon import AccustomedChameleonModel
...
def load_image(image_path: str):
    img_dir = "/data/dataset/align-anything-ti2ti"
    try:
        if image_path.startswith("http"):
            image = Image.open(requests.get(image_path, stream=True).raw)
        else:
            image_path = os.path.join(img_dir, image_path[2:])
...
def format_sample_cham(raw_sample: dict[str, Any]) -> dict[str, Any]:
    ...
    better_id = raw_sample.get("overall_response", 1)
    better_id = int(re.findall(r"\d+", better_id)[0])

    worse_id = 1 if better_id == 2 else 2

    input_text = raw_sample["question"]
    input_img = raw_sample["input_image"]

    better_text = raw_sample[f"text_response_{better_id}"]
    better_img = raw_sample[f"image_response_{better_id}"]

    worse_text = raw_sample[f"text_response_{worse_id}"]
    worse_img = raw_sample[f"image_response_{worse_id}"]

    # Align Anything dataset에서는 이미 이미지 토큰이 추가되어 있음
    ...
    better_text_full = (
        ...
        f"{assistant_prompt.format(output=better_text)}"
    )
    ...
    worse_text_full = (
        f"{system_prompt}"
        f"{user_prompt.format(input=input_text)}"
        f"{assistant_prompt.format(output=worse_text)}"
    )
...
def main():
    ...
    for id in range(num_processes):
        ...
        p = mp.Process(
            target=process_data, args=(gpu, chunks[id], model_path, output_paths, cache_path)
        )
        p.start()
        processes.append(p)
